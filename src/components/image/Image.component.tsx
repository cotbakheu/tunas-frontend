...
export default function Image(props: ImageProps): JSX.Element {
  const { src = '', alt = '', height, width, loading = 'lazy', className = '', isPlaceholder = false } = props
  const [imageRef] = useImageState()

  return (
    <div role='img' aria-label={alt} className={injectClassNames([className, !!className])}>
      {!isPlaceholder && <img src={src} alt={alt} ref={imageRef} height={height} width={width} loading={loading} />}
    </div>
  )
}
